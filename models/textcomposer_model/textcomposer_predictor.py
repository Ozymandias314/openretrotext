...
def _eval_decoding(args, model, device, dataset, result_file, pad_token_id, save_res=True):
    model.eval()
    pred_results = {}
    loader = CustomDataLoader(dataset, pad_token_id = pad_token_id, batch_size=args.batch_size, shuffle=False)
    ...
    for step, batched in enumerate(tqdm(loader, desc="Iteration")):
        ...
        with torch.no_grad():
            beam_nodes = model(batch, input_ids, attention_mask, typed=False, decode=True, beam_size=args.beam_size)
            cano_pred_mols = {}
...
class TextComposerPredictor:
    """Class for TextComposer Predicting"""
    ...
    def __init__(self,
        ...
        self.test_dataset = MoleculeDataset(args = self.model_args,
            root=processed_data_path, enc_tokenizer = self.enc_tokenizer, split='test', load_mol=True)
    ...
    def eval_decoding(self):
        ...
        _eval_decoding(args = self.model_args, model = self.model, device = self.device, dataset = self.test_dataset, result_file=result_file, pad_token_id = self.pad_token_id)
