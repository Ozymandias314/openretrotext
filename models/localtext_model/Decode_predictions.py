...
def main(args):   
    ...
    if args['model'] == 'default':
        result_name = 'localtext_%s.txt' % args['dataset']
    else:
        result_name = 'localtext_%s.txt' % args['model']
    ...
    with multiprocessing.Pool(processes=8) as pool:
        ...
        for result in tqdm(pool.imap_unordered(partial_func, tasks), total=len(tasks), desc='Decoding localtext predictions'):
    ...
    with open(output_path, 'w') as f1, open(output_path_class, 'w') as f2:
        for i in sorted(result_dict.keys()) :
            ...
            print('\rDecoding localtext predictions %d/%d' % (i, len(raw_predictions)), end='', flush=True)
    print ()
