...
from models.localtext_model.utils import load_dataloader, predict
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        smiles, bg, atom_labels, bond_labels, enc_inputs = batch_data
        if len(smiles) == 1:
            continue

        atom_labels, bond_labels, input_ids, attention_mask = atom_labels.to(args.device), bond_labels.to(args.device), enc_inputs["input_ids"].to(args.device), enc_inputs["attention_mask"].to(args.device)
        atom_logits, bond_logits, _ = predict(args, model, bg, input_ids, attention_mask)
...
class LocalTextTrainer(Trainer):
    """Class for LocalText Training"""
    ...
    def __init__(self,
        ...
        logging.info("Overwriting model args, based on original localtext training script")
        self.overwrite_model_args()
        misc.log_args(self.model_args, message="Updated model args")
        self.encoder_tokenizer = get_tokenizers(self.model_args)
    ...
    def train(self):
        ...
        filename = os.path.join(self.model_path, f"LocalText.pth")
        stopper = EarlyStopping(mode='lower', patience=args.patience, filename=filename)

        train_loader, val_loader, test_loader = load_dataloader(args, self.encoder_tokenizer)
