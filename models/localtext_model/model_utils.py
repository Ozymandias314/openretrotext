...
def unbatch_feats(bg, edit_feats, text_feats):
    sg = bg.remove_self_loop()
    gs = dgl.unbatch(sg)
    atom_feats = []
    bond_feats = []
    for i, g in enumerate(gs):
        ...
        atom_feats.append(torch.cat((edit_feats[i][:g.num_nodes()],
                          text_feats[i:i+1, :].expand( edit_feats[i][:g.num_nodes()].size()[0]  ,-1)), dim = 1  ))
        bond_feats.append(torch.cat((edit_feats[i][g.num_nodes():g.num_nodes()+g.num_edges()],
                        text_feats[i:i+1, :].expand(edit_feats[i][g.num_nodes():g.num_nodes()+g.num_edges()].size()[0], -1)), dim = 1))
