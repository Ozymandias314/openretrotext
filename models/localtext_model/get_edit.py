...
from models.localtext_model.utils import predict
    ...
    with open(os.path.join(args.test_output_path, "raw_results.txt"), 'w') as f:
        ...
        with torch.no_grad():
            for batch_id, data in enumerate(test_loader):
                smiles_list, bg, rxns, enc_inputs = data
                input_ids, attention_mask = enc_inputs["input_ids"].to(args.device), enc_inputs["attention_mask"].to(args.device)
                batch_atom_logits, batch_bond_logits, _ = predict(args, model, bg, input_ids, attention_mask)    
                sg = bg.remove_self_loop()
