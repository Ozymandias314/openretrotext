...
from models.localtext_model.Extract_from_train_data import get_full_template
from models.localtext_model.LocalTemplate.template_extractor import extract_from_reaction
from models.localtext_model.Run_preprocessing import get_edit_site_retro
from typing import Dict, List


class LocalTextProcessor(Processor):
    """Class for LocalText Preprocessing"""
    ...
    def match_templates(self):
        ...
        for phase, fn in [("train", self.train_file),
            ...
            with open(fn, "r") as csv_file:
                ...
                for row in csv_reader:
                    rxns.append(row["rxn_smiles"].strip())
                    nn_ids.append(row["nn_id"])
                    reactant_smiles.append(row["reactant_smiles"])
                    product_smiles.append(row["product_smiles"])
                #rxns = [row["rxn_smiles"].strip() for row in csv_reader]
